#' Create an Interactive Treemap (Version 3)
#'
#' \code{d3tree3} is the primary function for creating interactive d3.js treemaps from
...
#'
#' @export
d3tree3Output <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'd3tree3', width, height, package = 'd3treeR')
}

#' Widget render function for use in Shiny
#'
#' @export
renderD3tree3 <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, d3tree3Output, env, quoted = TRUE)
}
